    ...
    try {
      if (isSignUp) {
        if (inputValues.email && inputValues.password && inputValues.name) {
          const response = await handleSignupSubmit(inputValues);
        ...
        if (inputValues.email && inputValues.password) {
          const response = await handleLoginSubmit(inputValues);
          if (response.message) {
            setError(response.message);
          }
        } else {
        ...
        <div>
          ...
          <div>
            ...
            <p className="text-base sm:text-lg lg:text-xl text-gray-400 mb-6 sm:mb-8">
              {isSignUp
                ? "Join Trend Trove today and discover exclusive deals on your favorite products!"
                : "Welcome back to Trend Trove! Log in to continue your shopping journey."}
            </p>
          </div>
